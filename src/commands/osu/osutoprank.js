...
const modCalculator = require('./diffcalculator')
    ...
    async run(message, [args]) {
      ...
      if(!message.args[0]){
        ...
        osuApi.getUserBest({ u: usernameUser  }).then((scores) => {
          osuApi.getBeatmaps({ b: scores[0]["beatmapId"] }).then(async beatmaps => {
            ...
            await execOJSAMA(`curl https://osu.ppy.sh/osu/${scores[0].beatmapId} | node ojsama.js +${scores[0]['mods'].filter(e => e !== 'FreeModAllowed')} ${beatmaps[0]['maxCombo']}x ${AccCaculFixed}%` ,  async function (error, stdout, stderr) {
            let output = stdout.split('\n')
						let starRatingmods = await output[7]
            ...
            let embed = new MessageEmbed()
              ...
              if(modslist[0] !== undefined && modslist !== 'HD'){
                ...
                if(modslist.includes('DT') && modslist.includes('HR') === false){
                  let stats = modCalculator.calcProperties({
                    AR: parseFloat(callAR),
                    OD: parseFloat(callOD),
                    CS: parseFloat(callCS)
                }, modCalculator.mods.DoubleTime.value)
                    embed.addField(`**❯Map Stats with mods**`,`AR${(stats.AR).toFixed(1)} OD${(stats.OD).toFixed(1)} CS${(stats.CS)}`,true)
                  console.log(stats)
                    ...
                    OD: parseFloat(callOD),
                    CS: parseFloat(callCS)
                }, modCalculator.mods.HardRock.value)
                  embed.addField(`**❯Map Stats with mods**`,`AR${(stats.AR).toFixed(1)} OD${(stats.OD).toFixed(1)} CS${(stats.CS).toFixed(1)}`,true)
                } else if(modslist.includes('EZ') && modslist.includes('DT') === false) {
                  let stats = modCalculator.calcProperties({
                    AR: parseFloat(callAR),
                    OD: parseFloat(callOD),
                    CS: parseFloat(callCS)
                }, modCalculator.mods.Easy.value)
                  embed.addField(`**❯Map Stats with mods**`,`AR${(stats.AR).toFixed(1)} OD${(stats.OD).toFixed(1)} CS${(stats.CS).toFixed(1)}`,true)
                  ...
                  let stats = modCalculator.calcProperties({
                    ...
                    OD: parseFloat(callOD),
                    CS: parseFloat(callCS)
                }, modCalculator.mods.HardRock.value + modCalculator.mods.DoubleTime.value)
                  embed.addField(`**❯Map Stats with mods**`,`AR${(stats.AR).toFixed(1)} OD${(stats.OD).toFixed(1)} CS${(stats.CS).toFixed(2)}`,true)
                }
              }
