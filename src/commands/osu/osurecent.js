      ...
      if(!message.args[0]){
        ...
        osuApi.getUserRecent({u: usernameUser }).catch(async (e) => {
          message.reply(`${e}`)
            ...
            execOJSAMA(`curl https://osu.ppy.sh/osu/${scores[0].beatmapId} | node ojsama.js +${scores[0]['mods'].filter(e => e !== 'FreeModAllowed')} ${scores[0]['maxCombo']}x ${AccCaculFixed}% ${scores[0]['counts']['miss']}m` ,  async function (error, stdout, stderr){
            ...
            let embed = new MessageEmbed()
              ...
              if(modslist[0] !== undefined && modslist !== 'HD'){
                ...
                if(modslist.includes('DT') && modslist.includes('HR') === false){
                  ...
                  embed.addField(`**❯Map Stats with mods**`,`AR${(stats.AR).toFixed(1)}  CS${(stats.CS).toFixed(1)}`,true)
